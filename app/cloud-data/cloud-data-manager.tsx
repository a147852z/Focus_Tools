...
export default function CloudDataManager({ displayName, email, isAdmin, signOutHref }: { displayName: string; email: string; isAdmin: boolean; signOutHref: string }) {
  ...
  const floorDecor = normalizeFloorDecor(backup?.data.floorDecor, backup?.data.roomSize);
  ...
  return <main className="cloud-page">
    ...
    {backup === undefined ? <section className="cloud-panel"><p>正在讀取你的雲端備份…</p></section> : backup ? <>
      ...
      <section className="cloud-panel"><div className="cloud-panel-heading"><div><span>FLOOR TILES</span><h2>雲端地板收藏</h2></div><strong>已鋪設 {Object.keys(floorDecor.tiles).length} 格</strong></div>{floorCatalog.filter((floor)=>floor.price>0).map((floor)=><p key={floor.id}>{floor.name}：庫存 {floorDecor.stock[floor.id]??0} 格 · 已鋪設 {Object.values(floorDecor.tiles).filter((id)=>id===floor.id).length} 格</p>)}<small>庫存與每格位置包含在完整備份內，刪除雲端資料時一併刪除。</small></section>
