...
type FocusRecord = { id: number; taskId?: number; taskTitle: string; category?: string; minutes: number; coins: number; readingBonus?: number; completedAt: string };
...
export default function Home() {
  ...
  const [floorDecor, setFloorDecor] = useState<FloorDecor>(initialFloorDecor);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("focus-room-state", JSON.stringify({ tasks, coins, roomSize, roomZoom, inventory, placedFurniture, focusHistory, readingMinutes, floorDecor }));
  }, [tasks, coins, roomSize, roomZoom, inventory, placedFurniture, focusHistory, readingMinutes, floorDecor]);
  ...
  function addTask(event: FormEvent) {
    ...
    if (editingTask) {
      ...
      setTasks((current) => current.map((task) => task.id === editingTask.id ? { ...task, title: nextTitle, date: selectedDate, minutes: safeMinutes, rewardPoints: safeRewardPoints, category: newCategory } : task));
      setFocusTask((current) => current.id === editingTask.id ? { ...current, title: nextTitle, date: selectedDate, minutes: safeMinutes, rewardPoints: safeRewardPoints, category: newCategory } : current);
      setFocusHistory((current) => current.map((record) => record.taskId === editingTask.id || (!record.taskId && record.taskTitle === previousTitle) ? { ...record, taskId: editingTask.id, taskTitle: nextTitle, category: newCategory } : record));
      notify("任務已更新");
    } else {
  ...
  function settleFocus() {
    ...
    const sourceTask = tasks.find((task) => task.id === focusTask.id);
    const taskReward = sourceTask && !sourceTask.done ? Math.max(0, Math.floor(sourceTask.rewardPoints ?? 10)) : 0;
    const earned = focusedMinutes + readingBonus + taskReward;
    const record: FocusRecord = { id: Date.now(), taskId: focusTask.id, taskTitle: focusTask.title, category: focusTask.category, minutes: focusedMinutes, coins: earned, readingBonus, completedAt: new Date().toISOString() };
    if (focusTask.category === "閱讀") setReadingMinutes((value) => value + focusedMinutes);
    if (sourceTask && !sourceTask.done) setTasks((current) => current.map((task) => task.id === sourceTask.id ? { ...task, done: true } : task));
  ...
  function currentBackup() {
    return { version: 1, exportedAt: new Date().toISOString(), data: { tasks, coins, roomSize, roomZoom, inventory, placedFurniture, focusHistory, readingMinutes, floorDecor } };
  }
  ...
  async function importBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setTasks(restoredTasks); setCoins(Math.max(0,Math.floor(data.coins))); setRoomSize(restoredRoomSize); setRoomZoom(restoredRoomZoom); setInventory(restoredInventory); setPlacedFurniture(restoredPlaced); setFloorDecor(restoredFloorDecor); setFocusHistory(restoredHistory); setReadingMinutes(restoredReadingMinutes); setSelectedFurnitureUid(restoredPlaced[0]?.uid ?? null); notify("備份已成功匯入");
    } catch {
  ...
  function resetAllData() {
    if (!window.confirm("確定要清除所有任務、金幣、家具、地板收藏與專注紀錄嗎？此動作無法復原。")) return;
    hydrated.current = false;
    localStorage.setItem("focus-room-state", JSON.stringify({ tasks: [], coins: 0, roomSize: 6, roomZoom: 1, inventory: initialInventory, placedFurniture: [], focusHistory: [], readingMinutes: 0, floorDecor: initialFloorDecor }));
    window.location.reload();
  }
  ...
  return (
    <main className="app-shell">
      ...
      {screen === "today" && <TodayScreen tasks={todayTasks} completed={completed} progress={progress} readingMinutes={readingMinutes} onToggle={toggleTask} onStart={startFocus} onEdit={openEditTask} onAdd={() => openAddTask(todayKey)} />}
      ...
      {screen === "room" && <RoomScreen size={roomSize} zoom={roomZoom} inventory={inventory} placed={placedFurniture} floorDecor={floorDecor} onFloorChange={changeFloor} calibrations={furnitureCalibrations} selectedUid={selectedFurnitureUid} onSelect={setSelectedFurnitureUid} onPlace={placeFurniture} onMove={moveFurniture} onRotate={rotateFurniture} onZoomChange={setRoomZoom} onStore={storeFurniture} onExpand={expandRoom} onShop={() => setShopOpen(true)} />}
...
function ReadingRewardGuide() {
  return <div className="reading-reward-guide"><strong>如何獲得閱讀獎勵</strong><ol><li>新增或編輯任務時，將分類設為「閱讀」。</li><li>按「開始計時」，實際專注至少 1 分鐘。</li><li>計時結束後按「結算」，分鐘數才會加入累計。</li></ol><small>每累計 60 分鐘獲得 100 點；只勾選任務完成不會增加閱讀時間。</small></div>;
}

function TodayScreen({ tasks, completed, progress, readingMinutes, onToggle, onStart, onEdit, onAdd }: { tasks: Task[]; completed: number; progress: number; readingMinutes: number; onToggle: (id: number) => void; onStart: (task: Task) => void; onEdit: (task: Task) => void; onAdd: () => void }) {
  ...
  const minutesUntilReadingReward = 60 - readingProgress;
  return <section className="screen today-screen"><div className="date-row"><div><strong>{dateLabel}</strong><span>{weekdayLabel}</span></div><button type="button" onClick={onAdd} aria-label="新增任務">＋</button></div><div className="progress-copy"><span><strong>{completed} / {tasks.length}</strong> 今日完成</span><span>{progress}%</span></div><div className="progress-track" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><span style={{ width: `${progress}%` }} /></div><div className="reading-summary"><div><span>▤ 閱讀計時累計</span><strong>{readingMinutes} 分鐘</strong></div><div className="reading-progress" role="progressbar" aria-label="今天頁面的閱讀獎勵進度" aria-valuemin={0} aria-valuemax={60} aria-valuenow={readingProgress}><span style={{width:`${readingProgress/60*100}%`}} /></div><small>距下次 100 點獎勵還差 {minutesUntilReadingReward} 分鐘。</small><ReadingRewardGuide /></div><div className="section-heading"><h2>接下來</h2><button type="button" onClick={onAdd}>新增任務</button></div><div className="task-list">{tasks.map((task) => <article className={`task-row ${task.done ? "is-done" : ""}`} key={task.id}><button className="task-check" type="button" onClick={() => onToggle(task.id)} aria-label={`${task.done ? "取消完成" : "完成"}${task.title}`}>{task.done ? "✓" : ""}</button><div><strong>{task.title}</strong><span>{task.category} · {task.minutes} 分鐘 · ◆ {task.rewardPoints ?? 10} 點{task.category === "閱讀" ? " · 計入閱讀累計" : ""}</span></div><div className="task-actions">{task.done ? <span className="reward">完成 ＋{task.rewardPoints ?? 10}</span> : <button className="start-button" type="button" onClick={() => onStart(task)}>開始計時</button>}<button className="edit-button" type="button" onClick={() => onEdit(task)} aria-label={`編輯${task.title}`}>編輯</button></div></article>)}</div><aside className="room-hint"><span className="room-mark">◇</span><div><strong>再完成 2 項即可解鎖新地板</strong><p>完成任務，把進度變成房間裡看得見的收藏。</p></div></aside></section>;
}
...
function FloorSwatch({ id }: { id: FloorId }) {
  return <span className={`floor-swatch floor-surface floor-${id}`} aria-hidden="true">{Array.from({length:16},(_,index)=>{
    const x=index%4, y=Math.floor(index/4);
    return <span key={index} className={`iso-cell ${(x+y)%2===0?"tile-even":"tile-odd"}`} style={{left:`${50+(x-y)*12.5}%`,top:`${(x+y+1)*12.5}%`,width:"25%",height:"25%"}}/>;
  })}</span>;
}
...
function ProfileScreen({ tasks, coins, readingMinutes, roomSize, inventory, placed, focusHistory, cloudStatus, onCloudUpload, onCloudDownload, onExport, onImport, onReset, onInstall }: { tasks:Task[]; coins:number; readingMinutes:number; roomSize:number; inventory:Inventory; placed:PlacedFurniture[]; focusHistory:FocusRecord[]; cloudStatus:CloudStatus; onCloudUpload:()=>void; onCloudDownload:()=>void; onExport:()=>void; onImport:(event:ChangeEvent<HTMLInputElement>)=>void; onReset:()=>void; onInstall:()=>void }) {
  ...
  const focusRecordTitle=(record:FocusRecord)=>record.taskId ? tasks.find((task)=>task.id===record.taskId)?.title ?? record.taskTitle : record.taskTitle;
